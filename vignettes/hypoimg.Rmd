...
A pure version of the hamlet annotations can be added to any ggplot using the the functions `hypo_anno_l()` and `hypo_anno_l()`. These functions are simple wrappers around the **ggplot2** function `annotation_custom(()`.
...
Finally, the **hypoimg** package implements a new geom: `geom_hypo_grob()`.
